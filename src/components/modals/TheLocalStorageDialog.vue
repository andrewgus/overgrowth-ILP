	...
	<dialog role="alertdialog" ref="dialogEl" :class="$style.dialog">
		...
		<div :class="$style.dialog__btns">
			<template v-if="!confirmDelete">
				...
				<BaseButton text="Reset lesson?" isWarning @btnClick="doubleCheckDelete" />
			</template>
			<template v-else>
				<BaseButton text="Nevermind. Let&rsquo;s continue." @btnClick="dialogEl?.close()" />
				<BaseButton text="Yes. Reset this lesson." aria-describedby="doubleCheck" isWarning
