<template>
	<footer v-show="showFooter" :class="[$style.activitiesOff, { [$style.sendToBottom]: userUnderstands }]">
		...
		<div :class="$style.acknowledgeBtns">
			<BaseButtonVue aria-describedby="activitiesOffReminderMessage" text="I understand" @btnClick="acceptMessage"
				v-if="!userUnderstands" />
			<BaseButtonVue text="Back to top" isLink url="#titleHeading" />
		</div>
	</footer>
</template>
