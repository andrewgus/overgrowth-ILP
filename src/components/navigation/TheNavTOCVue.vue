<template>
	<BaseButtonVue :class="$style.btnTOC" :text="!isMenuOpen ? 'Go to&hellip;' : 'Close'" aria-haspopup="menu"
		:aria-expanded="isMenuOpen" aria-controls="navItemsList" @click="toggleMenu" />
	<transition :css="false" @before-enter="onBeforeEnter" @enter="onEnter" @before-leave="onBeforeLeave">
		<div id="navItemsList" v-show="isMenuOpen" :class="$style.lessonTOC">
			...
			<ol aria-describedby="additonalSections">
				<li v-for="navItem in $allNavSections" :key="navItem.id"
...
.lessonTOC {
	...
	>ol {
		...
		>li {
			...
			&::before {
				-webkit-transition: var(--timeShort) background-color var(--transitionFlourish);
